...
import { getDealerSales, getSubDealerSales, updateSale, createSale } from '../Dealers/services/dealerSalesService';
import { toast } from 'react-hot-toast';
import EditSaleModal from '../Dealers/components/EditSaleModal';
import { Edit, Search, Box, X, Eye } from 'lucide-react';
...
const DealerSales = () => {
    ...
    const fetchSales = async () => {
        ...
        try {
            setLoading(true);
            const salesFn = isSubDealer ? getSubDealerSales : getDealerSales;
            const [salesDataResponse, configResponse] = await Promise.all([
                salesFn(entityId),
                axios.get(`${import.meta.env.VITE_API_URL}/api/billing-config`),
            ]);
            const salesData = Array.isArray(salesDataResponse) ? salesDataResponse : [];
            setSales(salesData);
            setBillingConfig(configResponse.data);

            const uniqueModels = [...new Map(salesData
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Plumber Name</th>
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {paginatedModalSales.map((sale) => (
                                            <tr key={sale._id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(sale.soldAt).toLocaleDateString()}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">{sale.incentiveStatus?.replaceAll('_', ' ') || '-'}</td>
